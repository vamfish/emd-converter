...
### 改进
- **并行处理日志逐文件透传**：此前并行模式只有开头两行与最终汇总，中间
  完全静默（worker 的 print 都消失在子进程里）。新增
  `process_one_file_logged()` 捕获 worker 全部 stdout（分析 INFO、视角校正、
  ✓/✗ 产物清单）随结果回传，GUI 以 `✓ (n/总数) 文件名` 为头逐文件输出，
  与串行模式可见性一致；`process_one_file()` 保持原二元组签名（包装器），
  既有调用与测试不受影响
